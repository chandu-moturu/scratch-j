    ...
    <Paper elevation={3}>
        <div className='rounded text-center bg-purple-700 p-2 my-3'>
            <div className="grid grid-cols-2 my-2">
                ...
                <input type="text" value={state.message} onChange={(e)=>{e.target.value.length>0 && setState({...state,message: e.target.value})}} className="mx-2 p-1 py-0 text-center" />

            </div>
            <div className="flex text-center flex-row flex-wrap bg-purple-900 text-white px-2 py-1 my-2 text-sm cursor-pointer" onClick={()=>displayMessage()} id={comp_id}>
                {`Think ${state.message}`}
            </div>
        </div>
    </Paper>
...
export default connect(mapStateToProps) (Think)